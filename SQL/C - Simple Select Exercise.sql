...
SELECT StudentID , AVG(Mark) AS 'Avg Mark'
FROM Registration
GROUP BY StudentID
...
SELECT ClubId, COUNT(StudentID) AS 'Students'
FROM Activity
GROUP BY ClubId
...
SELECT CourseHours,AVG(CourseCost) AS 'Avg Cost'
FROM Course
...
SELECT ClubId, Count(StudentID) as 'Num of Students'
FROM Activity
GROUP BY ClubId